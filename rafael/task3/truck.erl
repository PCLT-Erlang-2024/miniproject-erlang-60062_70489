...
dispatch(TruckId, CurrentLoad, MainPid, NumTrucks, BeltPid) ->
    ...
    %% Generate a random delay between 1.5 and 3 seconds (in milliseconds)
    MinDelay = 1500, %% 1.5 seconds in milliseconds
    MaxDelay = 3000, %% 3 seconds in milliseconds
    ReplacementDelay = MinDelay + rand:uniform(MaxDelay - MinDelay),

    io:format("Truck ~p: Replacement delay is ~p milliseconds~n",
              [TruckId, ReplacementDelay]),
    timer:sleep(ReplacementDelay),
    NewTruckId = TruckId + NumTrucks,
    spawn(?MODULE, start, [NewTruckId, MainPid, NumTrucks, BeltPid]).
